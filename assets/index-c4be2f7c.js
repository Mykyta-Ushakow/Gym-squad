import{O as B,a as S,s as v}from"./modal-55d71e14.js";const x=localStorage.getItem("quoteData");let h="",f="",y="";if(x){const{quote:e,author:t,date:n}=JSON.parse(x);h=e,f=t,y=n}const q=new Date().toLocaleDateString(),P=q!==y;P?fetch("https://your-energy.b.goit.study/api/quote").then(e=>e.json()).then(e=>{h=e.quote,f=e.author,y=q,localStorage.setItem("quoteData",JSON.stringify({quote:h,author:f,date:y})),L(h,f)}).catch(e=>{console.error("Error",e)}):L(h,f);function L(e,t){const n=document.querySelector(".quote"),i=document.querySelector(".author");n.textContent=e,i.textContent=t}const b=Object.freeze({BASE_URL:"https://your-energy.b.goit.study/api",EXERCISE_ENDPOINT:"/exercises",RATING_ENDPOINT:"/rating",FILTERS_ENDPOINT:"/filters",QUOTE_ENDPOINT:"/quote",SUBSCR_EDPOINT:"/subscription"}),{BASE_URL:$,EXERCISE_ENDPOINT:M}=b;function A(){return window.matchMedia("(min-width: 768px)").matches?10:8}function O(e){const t=e.currentTarget;C(t.dataset.category,t.dataset.name).then(n=>{const i=document.querySelector(".filter-gallery");i.innerHTML=I(n.results),document.querySelectorAll(".exercises-btn").forEach(s=>s.addEventListener("click",B)),T(n,t.dataset.category,t.dataset.name)}).catch(console.log)}async function C(e,t,n=1){const i=new URLSearchParams({[e]:t,limit:A(),page:n}),l=`${$}${M}?${i}`;return(await S.get(l)).data}function I(e){return`<ul class="exercises-list">${e.map(R).join("")}</ul>`}function R({rating:e,name:t,burnedCalories:n,bodyPart:i,target:l,_id:s}){return`<li class="exercises-item">
